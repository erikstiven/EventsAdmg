...
const STATUS_LABELS: Record<string, { label: string; className: string }> = {
  ...
  'pendiente aprobación': { label: 'Pendiente aprobación', className: 'bg-amber-100 text-amber-800' },
  'pendiente de actualizacion': { label: 'Pendiente de actualización', className: 'bg-indigo-100 text-indigo-800' },
  'pendiente de actualización': { label: 'Pendiente de actualización', className: 'bg-indigo-100 text-indigo-800' },
  'pendiente completar': { label: 'Pendiente completar', className: 'bg-yellow-100 text-yellow-800' },
  ...
  completado: { label: 'Aprobado', className: 'bg-emerald-100 text-emerald-800' },
  rechazado: { label: 'Rechazado', className: 'bg-rose-100 text-rose-800' },
};

const normalizeStatus = (status?: string) => {
  if (!status) return 'pendiente aprobacion';
...
export default function PendingApprovals2() {
  ...
  const deriveStatus = (item: ApprovalItem) => {
    ...
    if (base === 'pendiente de actualizacion' || base === 'pendiente de actualización') return 'pendiente de actualizacion';
    if (base === 'rechazado') return 'rechazado';
    ...
    const anyMissingDocs = participants.some((p) => !(p.selfie && p.doc));
    if (approvedCount === participants.length) return 'aprobado';
    if (approvedCount > 0) return 'aprobado parcial';
    if (!anyMissingDocs) return 'pendiente aprobacion';
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-gray-600">
            Revisa los registros enviados y aprueba los que correspondan. El QR se enviará solo a los aprobados.
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {filtered.map((item) => {
                  const normalized = deriveStatus(item);
                  const statusStyle = STATUS_LABELS[normalized] || STATUS_LABELS['pendiente aprobacion'];
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Badge className={`${statusStyle.className} px-3 py-1 text-xs`}>
                          {statusStyle.label}
                        </Badge>
                      </TableCell>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-medium">
                      {STATUS_LABELS[deriveStatus(selected)]?.label || 'Pendiente'}
                    </div>
                  </div>
                  {detailMode === 'approve' && (
                    <div>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-center gap-2">
                          <Button
                            ...
                            variant="ghost"
                            title="Ver rostro"
                            aria-label="Ver rostro"
                            onClick={() => openPreview(`Rostro - ${p.name}`, p.selfie_url)}
                          >
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button
                            ...
                            onClick={() => openPreview(`Cédula - ${p.name}`, p.doc_url)}
                          >
                          ...
                          {detailMode === 'view' && (
                            <Button
                              size="icon"
                              variant="ghost"
                              title={p.approved && p.qr_token ? 'Ver QR' : 'QR disponible solo para aprobados'}
                              aria-label="Ver QR"
              ...
              <div className="rounded-lg border bg-white p-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-semibold">
                      {STATUS_LABELS[deriveStatus(updateTarget)]?.label || updateTarget.status}
                    </div>
                  </div>
                </div>
              </div>
