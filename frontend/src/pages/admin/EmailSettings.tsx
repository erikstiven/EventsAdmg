...
export default function EmailSettings() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        toast({
          title: 'Error',
          description: 'No se pudieron cargar las configuraciones de correo.',
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: 'Guardado',
        description: 'Configuración guardada. Reinicia el backend para aplicar cambios.',
      });
    } catch (error: any) {
      toast({
        title: 'Error',
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600">
            Configura el correo que enviará las invitaciones y el formato del mensaje.
          </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle>SMTP (Envío)</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2 text-sm text-gray-600">
              Este correo será el remitente de las invitaciones. Si usas Gmail, necesitas una contraseña de aplicación.
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <Input
                ...
                placeholder="EventAccess <correo@gmail.com>"
                disabled={loading}
              />
              <p className="text-xs text-gray-500 mt-1">
                Recomendado: usar el mismo correo del “Usuario”.
              </p>
            </div>
            <div className="md:col-span-2 flex items-center gap-2">
              <Switch
                ...
                disabled={loading}
              />
              <Label htmlFor="smtpTls">Usar conexión segura (recomendado)</Label>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Plantilla de invitación</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="text-sm text-gray-600">
              Escribe el mensaje que recibirán los invitados. Puedes usar variables.
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-2">
                Puedes usar negritas, colores, listas, links e imágenes desde la barra superior.
              </p>
              <p className="text-xs text-gray-500 mt-1">
                Ejemplo: Hola <code>{'{{nombre}}'}</code>, tu link es <code>{'{{link}}'}</code>.
              </p>
            </div>
            <div className="rounded-md border bg-white p-4">
              <div className="text-sm font-semibold mb-3">Vista previa (estilo correo)</div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Plantilla de QR (aprobación)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="text-sm text-gray-600">
              Este correo se envía cuando se aprueba a un invitado. Usa <code>{'{{qr_image}}'}</code> para insertar el QR.
            </div>
            <div>
              <Label>Asunto del correo</Label>
              <Input
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Ejemplo: Hola <code>{'{{nombre}}'}</code>, tu QR es <code>{'{{qr_image}}'}</code>.
              </p>
            </div>
            <div className="rounded-md border bg-white p-4">
              <div className="text-sm font-semibold mb-3">Vista previa (estilo correo)</div>
