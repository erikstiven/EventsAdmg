...
class InvitationGroupsService:
    ...
    DEFAULT_QR_EMAIL_TEMPLATE = (
        ...
        "<tr><td align='center'>"
        "<table role='presentation' width='620' cellspacing='0' cellpadding='0' style='width:620px;max-width:620px;background:#ffffff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;'>"
        "<tr><td style='background:#0f172a;padding:18px 24px;color:#fff;'>"
        "<div style='font-size:22px;font-weight:700;'>EventAccess</div>"
        "<div style='font-size:12px;color:#cbd5e1;margin-top:4px;'>Aprobación confirmada</div>"
        "</td></tr>"
        "<tr><td style='padding:24px;color:#0f172a;'>"
        "<p style='margin:0 0 10px 0;font-size:17px;'>Hola <strong>{{nombre}}</strong>,</p>"
        "<p style='margin:0 0 12px 0;font-size:14px;line-height:1.7;color:#334155;'>Tu registro para el evento <strong>{{evento}}</strong> fue aprobado.</p>"
        "<table role='presentation' width='100%' cellspacing='0' cellpadding='0' style='margin:0 0 16px 0;background:#ecfeff;border:1px solid #a5f3fc;border-radius:10px;'>"
        "<tr><td style='padding:12px 14px;font-size:13px;color:#155e75;'><strong>Estado:</strong> Aprobado<br><strong>Fecha de emisión:</strong> {{fecha}}</td></tr>"
        "</table>"
        "<div style='margin:0 0 12px 0;text-align:center;'>"
        "<div style='font-size:13px;color:#334155;margin-bottom:8px;'><strong>Tu código QR de ingreso</strong></div>"
        "<div>{{qr_image}}</div>"
        "</div>"
        "<div style='margin:0 0 12px 0;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 12px;font-size:12px;color:#1e3a8a;line-height:1.6;'><strong>Recomendación:</strong> lleva este QR en tu celular y un documento de identidad.</div>"
        "<div style='margin:0 0 12px 0;background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:10px 12px;font-size:12px;color:#9a3412;line-height:1.6;'><strong>Advertencia:</strong> el QR es personal. No lo compartas ni lo publiques.</div>"
        "<p style='margin:0;font-size:12px;color:#64748b;line-height:1.6;'>Si no visualizas el QR, abre este enlace:<br><a href='{{link}}' style='color:#2563eb;text-decoration:none;word-break:break-all;'>{{link}}</a></p>"
        "</td></tr>"
        "<tr><td style='padding:14px 24px;background:#f8fafc;border-top:1px solid #e2e8f0;'>"
        "<p style='margin:0;font-size:11px;color:#94a3b8;'>Correo automático de EventAccess. Si no reconoces este mensaje, ignóralo.</p>"
        "</td></tr>"
        "</table>"
        "</td></tr></table></div>"
    )
